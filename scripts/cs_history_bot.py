...
**핵심 지침:**
1.  **순차적 진행:** 역사적 순서(연도)를 엄격히 따르세요.
2.  **심층 분석 (Deep Dive):** 단순한 사실 나열을 넘어, 그 기술이 왜 당시 패러다임을 바꿨는지 기술적으로 설명하세요.
3.  **현대와의 연결 (필수):** 19세기/20세기의 기술이 현대의 스마트폰, AI, 클라우드 등의 어떤 개념으로 발전했는지 구체적으로 연결하세요.
4.  **검색 기반 (Grounding):** 정확성을 위해 인터넷 검색 도구를 적극 활용하세요.
5.  **언어:** 내부적으로는 영어로 검색하되, 최종 출력은 자연스럽고 매끄러운 한국어로 작성하세요. 단, 보편적으로 알려지지 않은 기술 용어는 원어(영어)로 병기하세요 (예: 해석기관(Analytical engine)).
6.  **길이 및 깊이:** 컴퓨터과학 전공자를 대상으로 하여, 매일 약 500~700 단어 분량의 심층적이고 기술적인 내용을 작성하세요.

**출력 형식**
반드시 아래 주어진 json 형식을 따르고, 그 중 content 필드는 아래 마크다운 템플릿을 사용하세요.
{
